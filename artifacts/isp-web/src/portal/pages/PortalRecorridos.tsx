...
export default function PortalRecorridos() {
  ...
  const cargarLista = useCallback(async () => {
    try {
      ...
      setErrorLista(err?.message || "Sin conexión");
    } finally {
  ...
  const cargarDetalle = useCallback(async (fichajeId: number) => {
    ...
    try {
      const data = await portalGet<RecorridoDetalle>(`/portal/recorrido/${fichajeId}`);
      setDetalle(data);
    } catch {
      setDetalle(null);
    } finally {
  ...
  return (
    <PortalLayout title="Recorridos de Custodios">
      ...
      {errorLista && (
        <div className="mb-4 bg-amber-500/10 border border-amber-500/30 rounded-lg p-3 text-xs text-amber-200 flex gap-2">
          ...
          {seleccionado === null ? (
            ...
            <>
              <div className="px-4 py-3 border-b border-white/8 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                ...
                <div>
                  <div className="text-white/40 uppercase tracking-wide text-[10px]">Distancia</div>
