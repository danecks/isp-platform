/**
 * services/nomina/generar-novedades.ts
 *
 * `generarNovedades(fecha, cierreId)` es el contrato formal entre Operaciones
 * (Fase 1: cierre operativo del día) y Nómina (Fase 2: pre-planilla / planilla).
 *
 * Originalmente vivía dentro de routes/nomina.ts (≈1.1k líneas). Se extrajo
 * a este servicio para separar la lógica del transporte HTTP. La lógica
 * (queries, pasos, regla de consistencia) NO cambió.
 *
 * IMPORTERS:
 *   - routes/nomina.ts             — re-exporta `generarNovedades` para
 *                                    compatibilidad con código que importa
 *                                    desde "../nomina"
 *   - routes/operaciones/cierre.ts — la usa al cerrar el día operativo
 */
