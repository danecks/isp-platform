...
// Lista todas las zonas con estadísticas (puestos, clientes, sedes, supervisor)
zonasRouter.get("/operaciones/zonas", async (req, res) => {
  try {
    ...
    const params: any[] = [];
    if (supervisorEmpId) {
      query += ` WHERE oz.supervisor_employee_id = $1`;
...
// Crear zona operativa global
zonasRouter.post("/operaciones/zonas", async (req, res) => {
  const { nombre, descripcion, supervisor_employee_id, supervisor_user_id } = req.body;
  ...
  try {
    const { rows } = await pool.query(
      `INSERT INTO operational_zones (nombre, descripcion, supervisor_employee_id, supervisor_user_id)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [nombre.trim(), descripcion || null, supervisor_employee_id || null, supervisor_user_id || null]
    );
...
// Actualizar zona (nombre, descripcion, supervisor, estado)
zonasRouter.patch("/operaciones/zonas/:id", async (req, res) => {
  const { nombre, descripcion, supervisor_employee_id, supervisor_user_id, estado } = req.body;
  ...
  try {
    const { rows } = await pool.query(
      ...
      [
        ...
        supervisor_employee_id !== undefined ? (supervisor_employee_id || null) : undefined,
        supervisor_user_id !== undefined ? (supervisor_user_id || null) : undefined,
        estado || null,
        req.params.id,
      ]
    );
