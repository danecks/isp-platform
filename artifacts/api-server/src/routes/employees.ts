...
const DIAS_VALIDOS = new Set(["domingo", "lunes", "martes", "miercoles", "jueves", "viernes", "sabado"]);

function normalizarDia(d: string): string {
  return d.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
}

employeesRouter.get("/employees/:id/descanso-semanal", async (req, res) => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) return res.status(400).json({ error: "id inválido" });
...
employeesRouter.put("/employees/:id/descanso-semanal", async (req, res) => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) return res.status(400).json({ error: "id inválido" });
...
employeesRouter.delete("/employees/:id/descanso-semanal/:semana", async (req, res) => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) return res.status(400).json({ error: "id inválido" });
  try {
