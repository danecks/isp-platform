...
import { Camera, CheckCircle2, ClipboardList, Star, ChevronDown } from "lucide-react";

type Stage = "scan" | "form" | "saved";
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col">
      ...
      <div className="bg-slate-900 border-b border-slate-800 px-4 py-3 flex items-center gap-3">
        ...
        <div className="ml-auto bg-indigo-900/50 border border-indigo-700/50 px-2 py-0.5 rounded-full">
      ...
      <div className="flex-1 overflow-y-auto">
        {stage === "scan" && (
          <div className="flex flex-col items-center justify-center p-6 gap-6 min-h-[600px]">
            <div className="relative w-52 h-52 rounded-2xl overflow-hidden bg-slate-900 border-2 border-indigo-500">
              <div className="absolute inset-0 bg-gradient-to-br from-slate-800 to-slate-900" />
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-36 h-36 border-2 border-indigo-400 rounded-xl relative">
                  ...
                  <Camera className="absolute inset-0 m-auto w-8 h-8 text-indigo-400/40" />
                </div>
              </div>
            </div>
            <div className="text-center">
              <p className="text-white text-lg font-bold mb-2">Escanear credencial</p>
              <p className="text-slate-400 text-sm leading-relaxed">
                Apunta la cámara al QR del reverso de la credencial del agente
              </p>
            </div>
            <div className="bg-slate-900 border border-slate-800 rounded-2xl px-4 py-3 w-full">
              <p className="text-slate-500 text-[10px] uppercase tracking-wider mb-2">Estado GPS</p>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
                <p className="text-green-400 text-xs">Ubicación verificada · 28 m del puesto</p>
              </div>
            </div>
            <button
              onClick={() => setStage("form")}
              className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-2xl text-sm transition-colors"
            >
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
              ...
              <div className="px-4 py-3 grid grid-cols-2 gap-x-4 gap-y-2">
                <div>
                  <p className="text-slate-500 text-[10px]">Nombre</p>
                  <p className="text-white text-sm font-semibold">Carlos Rodríguez</p>
                </div>
                <div>
                  <p className="text-slate-500 text-[10px]">ID</p>
                  <p className="text-slate-200 text-xs font-mono">AGT-2024-0147</p>
                </div>
                <div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl px-4 py-4">
              ...
              <textarea
                ...
                placeholder="Notas adicionales sobre el agente, situación del puesto, incidentes..."
        ...
        {stage === "saved" && (
          <div className="flex flex-col items-center justify-center p-6 gap-6 min-h-[600px]">
            ...
            <button
              onClick={() => { setStage("scan"); setChecks({}); setRating(0); setObs(""); }}
      ...
      <div className="bg-slate-900 border-t border-slate-800 px-4 py-2">
        <div className="flex gap-2 justify-center">
          {(["scan", "form", "saved"] as Stage[]).map((s) => (
            <button
              key={s}
              onClick={() => setStage(s)}
            ...
            >
              {s === "scan" ? "Escanear" : s === "form" ? "Formulario" : "Guardado"}
            </button>
          ))}
        </div>
      </div>
