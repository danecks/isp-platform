...
- **Advanced Modules:**
    ...
    - **Flujo Operaciones → RRHH → Planilla (HE Validation):** Complete approval chain for overtime (HE) payments. (1) When Operaciones registers a relevo/substitution, the system creates an `eventos_rrhh` record with `tipo_evento='horas_extra'` and `estado='pendiente_aprobacion'` for the covering agent (entrante), plus marks `novedades_nomina_diarias` with `requiere_revision_rrhh=TRUE` and `impacto_nomina='pendiente'`. (2) RRHH sees these in "Incidencias Pendientes" and resolves with `horas_extra_aprobadas` (approved → `impacto_nomina='aprobado_rrhh'`) or `horas_extra_rechazadas` (rejected → zeros out HE, `impacto_nomina='rechazado_rrhh'`). (3) Pre-planilla `QUERY_CONSOLIDADO` only counts `horas_extra` where `requiere_revision_rrhh IS NOT TRUE OR impacto_nomina='aprobado_rrhh'` AND `impacto_nomina <> 'pagado_efectivo'`; pending HE tracked separately as `horas_extra_pendientes`. Pre-planilla validation shows alerts for unresolved HE. (4) Cost estimation shown in substitution modal with "Requiere aprobación RRHH" indicator.
    - **Modo de Pago HE (ModalPagoHE):** After registering a relevo, the system shows a payment mode selector instead of the old "Incentivo Cash" modal. Two paths: (A) **En Planilla** — default flow, HE goes through RRHH approval chain before payroll inclusion; (B) **En Efectivo** — registers immediate cash payment via `incentivos_cash_cobertura` with `tipo='he_efectivo'`, marks `novedades_nomina_diarias.impacto_nomina='pagado_efectivo'` and resolves `eventos_rrhh` as `pagado_efectivo` — all in a single DB transaction. Prevents double-payment: the `pagado_efectivo` state excludes HE from both planilla and RRHH pending queues. Duplicate guard: only one `he_efectivo` record per employee+date+puesto (409 on retry). Tarifa pre-filled from `config_tarifa_he`.
