...
- **Servicios Programados — Vista Unificada (SP-01):** `servicios_programados_v` VIEW unifica clientes con `fecha_inicio_contrato` (tipo `inicio_cliente`, amber/Building2) y SSA activos/pendientes (tipo `ssa`, blue/Shield) en un único endpoint. `GET /proximos-arranques` y `pool-futuro iniciosProyecto` devuelven ambos tipos. Banner y PoolFuturoPanel muestran diferenciación visual.
- **Planificación Futura SSA (PF-02):** `planificacion_futura` ampliada con `puesto_id` nullable, `ssa_id VARCHAR(30)`, `tipo_cobertura_futura TEXT`. `GET /planificacion-futura?fecha=` usa LEFT JOINs e incluye planes de tipo SSA. `POST /planificacion-futura` acepta `ssaId` alternativo a `puestoId`. Pool-futuro enriquece iniciosProyecto SSA con `plan_id`, `plan_relevo_id`, `plan_relevo_nombre`. `ModalPlanSSA` permite asignar/editar/cancelar planes de cobertura para SSA futuros directamente desde el PoolFuturoPanel. Tarjetas SSA muestran badge verde "Planificado" + nombre de agente si hay plan, o "Sin agente asignado" si no. Tablero SSA muestra `plan_agente_nombre` (agente pre-asignado para hoy). `invalidateFuture()` invalida también la query `["pool-futuro"]` para que el panel se refresque tras guardar/cancelar planes.
- **SelectorAgenteAgrupado (universal):** Componente compartido que reemplaza todos los selectores de texto libre en Operaciones. Agrupa agentes por estado operativo real: disponible (verde), descansando (azul + badge HE), en_puesto (teal), en_ssa (ámbar), ausente (rojo, no seleccionable). Para fechas futuras usa `/pool-futuro?fecha=`; para hoy usa `/pool`. Confirmación inline para doble asignaciones. Usado en: `ModalPlanSSA` (slots de guardia SSA), `ModalAsignarSSA` (agregar agentes a SSA del día), `ModalPlanFuturo` (relevo programado de puestos), `ModalSegmentos` (tramos de cobertura).
- **Titularidad Histórica Temporal — TH (conectada a operación real):** La titularidad dejó de ser un valor estático (`titular_employee_id` en `puestos_operativos`) y se convierte en una dimensión temporal real usando `puesto_titular_historico`. Regla: para cualquier fecha X, el titular de un puesto es el registro en `puesto_titular_historico` donde `fecha_inicio <= X AND (fecha_fin IS NULL OR fecha_fin >= X)`. Fallback: si no existe registro histórico, se usa `puestos_operativos.titular_employee_id`. Módulos actualizados: (1) `GET /operaciones/tablero` — pizarrón usa titular histórico por `?fecha=`; (2) `POST /operaciones/cierre` — snapshot retroactivo y snapshot de hoy usan titular histórico de la fecha cerrada; (3) `generarNovedades()` P-NOM-04 — detección de ausencias de titulares usa el titular histórico de la fecha procesada (no el actual); (4) Paso 1 y 1.5 de nómina — `puesto_titular_id` almacenado en novedades usa historial por fecha; (5) `pre-planilla.ts` LATERAL IGSS — busca el puesto del que el colaborador fue titular durante el período usando `puesto_titular_historico`, con fallback a `titular_employee_id` actual.

- **Motor de Turnos Completo — 5 tipos ISPSA (T001-T007):** `turno-calc.ts` reescrito con `calcularEstadoCiclo()` que soporta 12h (diario), 24x24 (48h ciclo), 24x48 (72h ciclo), 24x72 (96h ciclo, nuevo), 8x8 (16-días ciclo, nuevo). Retorna `{trabaja, horasEsperadas, descansoPorCiclo, disponibleHE, tipoCiclo}`. `descansoPorCiclo=true` distingue descanso normal del ciclo de ausencias reales. DB: columna `tipo_ciclo` añadida a `turnos`; turnos id=271 (24x72) e id=272 (8x8) insertados. Tablero: devuelve `descanso_por_ciclo` boolean por puesto. Pool-futuro: `calcularEstadoTurno()` reemplazada por wrapper del motor real; agentes en descanso normal llevan `descansoPorCiclo:true` y `disponibleHE:true`. Frontend Pizarrón: tarjetas `descansoCiclo` con borde índigo/azul + ícono Luna (vs. borde rojo + Circle pulsante para descubiertos reales). Header de cliente muestra "N en ciclo · N descubiertos" separados. Validado con datos de prueba: 6 agentes 24x72 (Banco Industrial), 2 agentes 8x8 (Ind. Quetzal), etiquetados `stress_test_2026`.
