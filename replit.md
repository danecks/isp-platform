...
- **Planilla Final (`/admin/rrhh/planilla`) + Control financiero completo:** Full payroll cycle closure with financial control. DB: `planillas` (header, anulada soft-delete, partial unique index WHERE anulada=FALSE), `planilla_lineas` (full breakdown + anticipo_ids/novedad_ids/segmento_ids JSONB trazabilidad + igss_trabajador/igss_patronal/otros_descuentos reserved columns). `pre_planilla_cierres` has `anulado/anulado_por/anulado_at` to allow reversal+reopen. Endpoints: GET /nomina/planillas, POST /nomina/planilla (generates from snapshot + links anticipos), GET /nomina/planilla/:id, PATCH /nomina/planilla/:id/estado, DELETE /nomina/planilla/:id (reversal: unlinks anticipos, marks cierre as anulado, reopens revision period), GET /nomina/planilla/:id/export (CSV). Anti-double-discount: on planilla generation, anticipos matched by employee_id (estado pendiente/aprobada, planilla_id NULL) are linked (planilla_id set, estado→descontado). PATCH /anticipos/:id checks planilla_id NOT NULL and rejects with 409. Reversal flow: DELETE unlinks anticipos (→aprobada), marks cierre anulado, reopens periodo_cerrado=FALSE, allowing re-close and re-generate. Pre-planilla cierre queries updated with AND anulado=FALSE to support this cycle. Frontend: RevertirPlanillaModal with "REVERTIR" typed confirmation, anticipo_ids shown in Anticipos tab with linked count. Current limitations: no IGSS, no bonificación incentivo, no séptimo.
- **Pre-Planilla Control de Cierre (v3):** Extended the pre-planilla module into a formal pre-close control system. New features: (1) 4-state validation per collaborator (pendiente/revisada/observada/aprobado_rrhh) with audit trail in `pre_planilla_auditoria`; (2) automatic validation engine (GET /validacion) detecting critical errors (falta+trabajo same day, hours worked without time, etc.) and alerts (active employees without records); (3) "Cerrar período" button with blocking modal that shows validation results and prevents close if there are critical errors; (4) period snapshot saved to `pre_planilla_cierres` table (JSONB), freezing data for audit and planilla final reference; (5) edit-lock: after period close, revision changes are rejected with 409; (6) validation panel inline above the tab navigator showing errors/alerts in real time.
...
- **Tipos de Novedad Estructurados (13 categorías — MOV-01/02/03):** Replaces free-text `motivo` with a structured `tipo_novedad` field on `cobertura_segmentos` and `novedades_nomina_diarias`. DB migrations add `tipo_novedad` (VARCHAR 60) and `cobertura_alcance` to `cobertura_segmentos`, `tipo_novedad` to `novedades_nomina_diarias`, and `estado_operativo_puesto` to `puestos_operativos`. The 13 categories (`falta_total`, `abandono_parcial`, `vacaciones`, `incapacidad`, `suspension`, `permiso_con_goce`, `permiso_sin_goce`, `relevo_completo`, `relevo_parcial`, `relevo_vacaciones`, `horas_extra_puras`, `ssa_externo`, `cambio_titular`) are organized into 4 payroll-impact groups: con_descuento, sin_descuento, cobertura, especial. `generarNovedades()` reads `tipo_novedad` from relevo segments and propagates it to `novedades_nomina_diarias`, adjusting `falta`/`suspension`/`descuento_dia` (e.g., `vacaciones`/`incapacidad` → no falta, no descuento). `estado_operativo_puesto` is updated per substitution. Frontend `ModalSustitucion` shows a rich grouped-button picker (color-coded by payroll impact). PuestoCard shows dynamic badge (RELEVO / VACACIONES / INCAPACIDAD / SUSPENSIÓN / etc.). PrePlanilla detail panel and Anexo Faltas table show `tipo_novedad` label with color coding.
...
- **SSA Pipeline Sync (Phase 2):** When a tarea linked to an SSA is closed via `POST /tareas/:id/cerrar`, the system automatically syncs the corresponding area state (`estado_operaciones → 'cubierta'`, `estado_rrhh → 'viable'`, `estado_comercial → 'facturado'`) and recalculates `estado_general` using a shared `calcEstadoGeneralSSA()` function. State machine: cancelada if ops=no_viable; cerrada if comercial=cobrado; pendiente_facturacion if comercial=facturado|pendiente_cobro; cubierta if all areas ready; pendiente_operaciones/rrhh/en_revision as intermediate states.
- **Pool de Agentes — Elegibilidad y Sincronización (SSA-04):** Added `elegible_pool BOOLEAN DEFAULT TRUE` column to employees. Migration SSA-04 auto-marks supervisors, jefes, directors, and non-operational areas (supervisión, administración, rrhh, comercial, contabilidad) as `elegible_pool = FALSE`. Pool query now: (1) excludes non-eligible profiles via WHERE filter, (2) adds LEFT JOIN on active SSA assignments to detect agents currently covering a service, (3) returns new `enSSA` group alongside `disponibles/enPuesto/enDescanso/suspendidos`. Frontend pool panel shows "En SSA" tab (amber); SSA-assigned agents are excluded from disponibles and non-draggable. `idsEnPuesto` set in ModalAsignarSSA now also includes SSA agents.
- **Pool Faltando — Bug Fix:** Fixed pool query WHERE clause in `/api/operaciones/pool`. Employees with `elegible_pool=FALSE` who are titulares of puestos now appear in the Faltando tab when they have an active absence (`estado_operativo_puesto != 'normal'`). Previously these titulares (e.g. supervisors/fixed-post guards) were invisible in the Faltando tab. Fix: Added OR branch `(titular_po.id IS NOT NULL AND titular_po.agente_id != e.id AND estado_operativo_puesto != 'normal' AND estado_laboral = 'activo')` to the WHERE clause.
- **Pre-Planilla GROUP BY Fix:** Added missing IGSS columns (`aplica_igss_general`, `estado_igss`, `fecha_inicio_igss`, `po.aplica_igss`, `po.regimen_igss`) to the GROUP BY clause in the pre-planilla SQL query template. This resolved a PostgreSQL aggregation error that caused the pre-planilla endpoint to return 500 when employees had IGSS data.
- **Arquitectura Operativa Unificada:** El **Pizarrón Operativo** (`/admin/operaciones`) es el único centro de operación diaria — gestiona puestos normales, coberturas, relevos, ausencias, y servicios especiales (SSA) desde el pool. El antiguo "Tablero de Servicios" se convirtió en **"Seguimiento SSA"** (`/admin/tablero-servicios`): vista administrativa de solo lectura/seguimiento para resúmenes finales, contabilidad/facturación y estado por área departamental. La pestaña "Asignar Agente" fue eliminada de esta vista para evitar duplicidad funcional. El **Pipeline SSA** (`/admin/pipeline-servicios`) se mantiene como vista de estado general del pipeline comercial.
