...
- **Matriz de Permisos por Módulo y Rol — PERM-01 (Editable, dinámica):** Nueva tabla `permisos_ruta_rol` (UNIQUE path+rol) con 75 entradas semilla mapeando NAV_SECTIONS a roles. `GET /api/config/permisos` devuelve mapa `{path → roles[]}`. `PUT /api/config/permisos` (solo admin) reemplaza permisos no-admin atómicamente. Hook `usePermisos()` carga desde API con React Query (staleTime 5min, fallback estático). `AdminSidebar` y `AuthGuard` usan el hook para permisos dinámicos. La tabla estática hardcodeada en `Usuarios.tsx` fue reemplazada por `PermisosRolMatrix`: matriz editable con columnas Admin/Operaciones/RRHH/Comercial/Supervisor, agrupada por sección, checkboxes interactivos (Admin siempre marcado/bloqueado), botón "Guardar cambios" con toast de confirmación. Cambios persisten en DB y el sidebar los refleja en el siguiente reload.
