...
- **Advanced Modules:**
    ...
    - **Módulo Custodias v2 (CUST-03):** Complete custodia service module. Clients marked `tipo_servicio='custodia'` are excluded from pizarrón and managed in `/admin/custodias`. New tables: `custodia_fuerza_semanal` (daily force schedule per client, day 0-6 → agent count), `custodia_asignacion_diaria` (daily agent assignments per client). Dashboard shows all custodia clients with force/assigned/pending counts. Agent assignment panel with pool search. Fuerza semanal editor per client. Printable deployment sheet (hoja imprimible) matching physical form: Marca, Serie, Código (blank—client fills), No. (blank—client fills), Nombre, Munición, Firma. `tipo_servicio` selector in FichaCliente (vigilancia/custodia/mixto). Pizarrón filter: `COALESCE(cl.tipo_servicio,'vigilancia') != 'custodia'`.
