...
- **Solicitudes de Eliminación (`/admin/solicitudes-eliminacion`) — DEL-01:** Admin page for reviewing and managing deletion requests. Filter tabs: Todas/Pendiente/Aprobada/Rechazada with counts. Inline approve/reject confirmation flow. PATCH API integration. Table: `solicitudes_eliminacion`. Sidebar nav item under "Sistema" (admin only). Bug fix: `a.id AS arma_id` alias in `armeria.ts` estado-operativo query returned `undefined` for `arma.id` at runtime — fixed by adding non-aliased `a.id` alongside `a.id AS arma_id`.
- **Cancelación de Servicios Adicionales (SSA-CAN-01):** Added cancel workflow for SSA (solicitudes_servicio_adicional). Migration adds `motivo_cancelacion TEXT`, `cancelado_por VARCHAR(100)`, `cancelado_at TIMESTAMPTZ` columns. New endpoint `PATCH /api/solicitudes-servicio/:id/cancelar` validates state (blocks cancelación de cerradas/ya canceladas), records who cancelled and motivo. Frontend: red "Cancelar este servicio" link in modal footer (hidden for already-cancelled/closed), expands into inline confirmation panel with optional motivo textarea and confirm/no buttons. If already cancelled, modal footer shows the cancelación info (by whom + motivo).
- **Novedades RRHH — Flujo de Revisión Pendiente (NRRHH-01):** Full pending-review workflow for daily absences and incidents. Instead of automatically setting `falta=TRUE, descuento_dia=TRUE` at pizarrón liberation, records now land with `impacto_nomina='pendiente', requiere_revision_rrhh=TRUE`. RRHH reviews and classifies via a new "Incidencias" tab in the RRHH Eventos page (`/admin/incidencias`). New migration adds `impacto_nomina VARCHAR(30)`, `requiere_revision_rrhh BOOLEAN`, `evento_rrhh_id INTEGER FK` to `novedades_nomina_diarias`; adds `tipo_resolucion`, `afecta_nomina`, `cantidad_horas`, `cantidad_dias`, `afecta_septimo_res`, `rrhh_resuelto_por/at` to `eventos_rrhh`. New API endpoints: `GET /api/rrhh/incidencias/pendientes`, `GET /api/rrhh/incidencias/pendientes/count` (badge), `PATCH /api/rrhh/incidencias/:id/resolver`, `GET /api/rrhh/incidencias/historial`. 8 resolution types mapped to correct `falta/suspension/descuento_dia/afecta_septimo` values. Pre-planilla behavior preserved — only affects `falta/descuento_dia` after RRHH approval. Frontend: new "Incidencias" amber tab with live badge counter in RRHHEventos.tsx; IncidenciasTab and ModalResolverIncidencia components.
- **Integración Custodios ↔ Puestos (CUST-02) — `tipo_puesto`:** Migration CUST-02 adds `tipo_puesto VARCHAR(20) DEFAULT 'normal' CHECK IN ('normal','custodia')` to `puestos_operativos`. All existing puestos remain `normal`. POST/PATCH `/api/operaciones/puestos` accept `tipoPuesto` param. `ModalNuevoPuesto` in Operaciones.tsx now has a two-button toggle (Operativo normal / Custodia) with an informational note when Custodia is selected. Toast on creation includes "· Custodia" suffix. Pizarrón card (DroppablePuesto) shows amber "CUSTODIA" badge next to puesto name for tipo_puesto='custodia'. New endpoint `GET /api/custodias/puestos` returns only custodia puestos with computed `estado_custodia` (incidente_activo/incidente_completado/en_ruta/planificada based on incidents.estado and agente_id). Custodias.tsx fully rewritten with real API data: 4 KPI stat cards, alert banner for incidentes activos, "En Ruta" highlight cards, filterable table with 7 columns (Puesto/Cliente/Agente/Zona/Turno/Estado/Incidentes), auto-refresh every 60s, empty-state guidance. Puesto interface updated with `tipo_puesto` field.
- **Nómina — Motor de Cálculo Completo (nomina-calc.ts):** `nomina-calc.ts` is the SINGLE SOURCE OF TRUTH for all payroll calculations. Shared by pre-planilla (estimate) and planilla final (exact). Guarantees zero divergence between `cierre.total_estimado` and `planilla.total_bruto`. Formula: `sueldoDia=sueldo_base/30`, `horasDia=horas_contrato/6` (or 8 if zero), `sueldoPeriodo=sueldoDia×days` (or `sueldo_base` for monthly second-half), `descFaltas=sueldoDia×(faltas+suspensiones)`, `valorHE=(sueldoDia/horasDia)×1.5×horas_extra`, `totalBruto=sueldoPeriodo−descFaltas−descSeptimo+valorHE`.
- **Nómina — Séptimo Día RRHH-Driven (SEP-01):** Séptimo día deduction is authority-driven: RRHH decides per-event via `eventos_rrhh.afecta_septimo_res=TRUE`. `QUERY_CONSOLIDADO` counts distinct ISO weeks in period where RRHH flagged loss → `septimos_perdidos`. `descSeptimo=sueldoDia×septimos_perdidos`. Stored in `planilla_lineas.desc_septimo` and `planillas.total_desc_septimo`. Migration SEP-01 adds: `impacto_septimo` to `eventos_rrhh`, `desc_septimo` to `planilla_lineas`, `total_desc_septimo` to `planillas`. Pre-planilla re-closure after PLAN-CUN-01 partial index allows same-period re-close after anulación.
- **Nómina — IGSS Guatemala (IGSS-02):** IGSS computed in `calcularLinea()` at planilla generation time using current DB state (`employees.aplica_igss_general`, `estado_igss='activo'`, `puestos_operativos.aplica_igss`). Rates: trabajador 4.83%, patronal 12.67% of `total_bruto`. IGSS does NOT apply on bonificación incentivo. Migration IGSS-02 adds `total_igss_trabajador` and `total_igss_patronal` to `planillas`. Planilla formula: `total_neto=total_bruto−igss_trabajador+bonificacion_incentivo−anticipos`.
- **Nómina — Bonificación Incentivo Decreto 78-89 (BONO-01):** Cálculo SIEMPRE proporcional a días pagables. Función `calcularBonificacionIncentivo()` en `nomina-calc.ts` (fuente única de verdad). Fórmula: `montoBase × (diasPagables / diasPeriodo)`. montoBase = Q125 (quincenal) · Q250 (mensual). `diasPeriodo` = días calendario del período inclusive ambos extremos. `diasPagables` = dias_trabajados + dias_vacaciones + dias_permiso_con_goce + dias_incapacidad (capped a diasPeriodo, min 0). NO incluye permiso_sin_goce, ausencias injustificadas ni suspensiones. Resultado redondeado a 2 decimales. Sin bandera de configuración — bono proporcional es la única forma válida. Migration BONO-01 adds `bonificacion_incentivo` to `planilla_lineas` and `total_bonificacion_incentivo` to `planillas`.
- **Nómina — Contadores de Días Especiales en QUERY_CONSOLIDADO:** `QUERY_CONSOLIDADO` now includes four day counters sourced from `novedades_nomina_diarias.tipo_novedad`: `dias_vacaciones`, `dias_incapacidad`, `dias_permiso_sin_goce`, `dias_permiso_con_goce`. These appear in the GET `/api/nomina/pre-planilla` response and the cierre snapshot for reporting and audit purposes.
- **Nómina — Suite E2E Integral (E2E-ABR-2026):** 99-test comprehensive suite (`pnpm --filter @workspace/api-server test:e2e-payroll-april`). Files: `artifacts/api-server/src/test/e2e-abril/`. Components: `seed.ts` (24 employees, 2 sedes, 8 scenarios, 360 novedades, 6 eventos RRHH — all seeded transactionally with MARKER=`E2E-ABR-2026`), `unit-tests.ts` (36 unit tests for `calcularBonificacionIncentivo`, `calcularBruto`, IGSS rates, and day isolation), `validate.ts` (63 E2E validations V-01→V-36 covering data integrity, séptimo isolation, vacaciones/permiso/HE counters, IGSS eligibility, period boundaries, coverage linkage, bonification per-employee, idempotency), `run.ts` (orchestrator). Totals validated against DB: bruto Q48,545 · IGSS-T Q733.37 · IGSS-P Q1,923.73 · bono Q3,358.35 · neto Q51,169.98.
- **Módulo Prestaciones Laborales (`/admin/rrhh/prestaciones`) — PREST-01:** Complete Guatemala labor benefits module. Backend: pure calculation library `prestaciones-calc.ts` (aguinaldo Dec1-Nov30, bono14 Jul1-Jun30, vacaciones 15d/20d quinquenio, indemnización despido_injustificado only, liquidación final, provisiones). Migration PREST-01 adds 8 tables: `prestaciones_config`, `prestaciones_acumulados`, `prestaciones_movimientos`, `prestaciones_provisiones`, `prestaciones_liquidaciones`, `prestaciones_liquidacion_detalle`, `vacaciones_saldos`, `vacaciones_movimientos`. 14 REST endpoints at `/api/prestaciones/*`. Test suite: 94/94 passing (53 unit + 41 E2E, run with `pnpm --filter @workspace/api-server test:prestaciones`). Frontend: 4-tab page (Vacaciones, Provisiones, Liquidaciones, Configuración). Provisiones: idempotent calculation per period per employee, skips null/zero salaries. Liquidaciones: simulate+confirm flow with anti-duplicate 409 guard, anulation support. Vacaciones: saldo tracker with movimientos ganadas/gozadas. Known route pattern: routes registered without `/api` prefix (router mounted at `/api` in app.ts).
