...
- **Advanced Modules:**
    ...
    - **Fichaje QR de Agentes (FICHAJE-QR-01):** QR credential system for guard attendance and supervisor inspection. Each guard has a unique UUID token on their credential card (printable). Scanning the QR opens `/agente?token=<uuid>`. Logic: if the user is NOT logged in (or logged in but not supervisor/admin), shows the fichaje (check-in) flow — captures GPS, validates proximity to the configured post (haversine, configurable radius), records attendance. If the user IS logged in as supervisor/admin, shows a supervision form (checklist, star rating, observations). Tables: `agente_qr_tokens` (UUID token per employee, one active per person), `puestos_gps` (GPS lat/lng + radius per puesto_operativo), `agente_fichajes` (unified log for both fichajes and supervisiones, with JSONB checks, calificacion 1–5, observaciones, supervisor_nombre). Admin UI: FichajeQR.tsx at `/admin/fichaje-qr` — two tabs: (1) Credenciales: list of all active agents, generate/regenerate/revoke token, print QR card; (2) Historial: scrollable log of fichajes and supervisiones with filter by tipo. Public page: AgenteEscaneo.tsx at `/agente?token=<uuid>` — auto-requests GPS, submits fichaje or supervision. Supervisor selector tabs appear only when logged in. API: GET `/api/agente/tokens`, POST `/api/agente/tokens/generate`, DELETE `/api/agente/tokens/:id`, GET `/api/agente/scan/:token` (public), POST `/api/agente/fichaje` (public + GPS validation), POST `/api/agente/supervision` (requires session), GET `/api/agente/fichajes`, GET/PUT `/api/puestos-gps`. Permissions module: `fichaje_qr` (roles: admin, operaciones, supervisor).
