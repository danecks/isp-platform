...
- **Planilla Final (`/admin/rrhh/planilla`):** Closes the full payroll cycle. Generates the formal payroll from a closed pre-planilla snapshot (JSONB stored in `pre_planilla_cierres`). New tables: `planillas` (header with totals, estado workflow, cierre_id FK) and `planilla_lineas` (one row per collaborator with full calc breakdown). Endpoints: GET /nomina/planillas, POST /nomina/planilla (generates from snapshot), GET /nomina/planilla/:id (detail + lines), PATCH /nomina/planilla/:id/estado (borrador→revisada→aprobada→pagada), GET /nomina/planilla/:id/export (CSV with BOM). Calculation: sueldoDia=sb/30, sueldoPeriodo=sueldoDia×periodoTotalDias, descFaltas=sueldoDia×(faltas+susp), valorHE=(sueldoDia/horasDia)×1.5×HE, totalBruto=sueldoPeriodo-descFaltas+valorHE, totalNeto=totalBruto-anticipos. Current limitations: no IGSS, no bonificación incentivo, no séptimo. Frontend: 4-tab view (Planilla General, Horas Extra, Faltas/Descuentos, Anticipos), KPI cards, audit trail via `pre_planilla_auditoria`, read-only (corrections go back through pre-planilla).
